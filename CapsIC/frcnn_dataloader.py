...
class DataLoader():
    def __init__(self,opt):
        ...
        if self.num_boxes==36:
            self.feature_path = "/home/data/COCO/DATA/image_features/frcnn_features/"
        elif self.num_boxes==100:
            self.feature_path = "/home/data/COCO/DATA/image_features/frcnn_features_100/"
    ...
    def load_captions(self,splits):
        ...
        for split in splits:
            for imgid in self.split_ix[split]:
                ...
                if split == 'train':
                    featFileExist = False
                    if os.path.exists(self.feature_path+"train/"+str(imgid)+".npy"):
                    # and os.path.exists(self.avg_feat_path+"train/"+str(imgid)+".npy"):
                        featPath = self.feature_path+"train/"+str(imgid)+".npy"
                        #avgPath = self.avg_feat_path+"train/"+str(imgid)+".npy"
                        featFileExist = True
                         
                    elif os.path.exists(self.feature_path+"val/"+str(imgid)+".npy"):
                    # and os.path.exists(self.avg_feat_path+"val/"+str(imgid)+".npy"):
                        featPath = self.feature_path+"val/"+str(imgid)+".npy"
                        #avgPath = self.avg_feat_path+"val/"+str(imgid)+".npy"
                        featFileExist = True
                        
                    assert featFileExist==True
                else:
                    featFileExist = False
                    if os.path.exists(self.feature_path+split+"/"+str(imgid)+".npy"):
                    #and  os.path.exists(self.avg_feat_path+split+"/"+str(imgid)+".npy"):
                        featPath = self.feature_path+split+"/"+str(imgid)+".npy"
                        #avgPath = self.avg_feat_path+split+"/"+str(imgid)+".npy"
                        featFileExist = True
                    assert featFileExist==True
    ...
    def get_batch(self,split,batch_size=None):
        ...
        for i in range(batch_size):
            ...
            feature_batch[i,:tmp.shape[0]] = tmp
            #avg_feat_batch[i*self.seq_per_img:(i+1)*self.seq_per_img] = np.load(item['avg_feat_path'])
    ...
    def get_test_batch(self,split='test',batch_size=None):
        ...
        for i in range(batch_size):
            ...
            imgids.append(imgid)
            feature_path = self.feature_path+"val/"+str(imgid)+".npy"
